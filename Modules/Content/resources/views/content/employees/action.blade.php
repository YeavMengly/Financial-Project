@if (hasPermission('employees.edit') or hasPermission('employees.destroy'))
    <div class="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
            ...
            @if (is_null($module->deleted_at))
                @if (hasPermission('employees.edit'))
                    <a href="{{ route('employees.edit', encode_params($module->id)) }}" class="dropdown-item"><i
                            class="bx bx-edit"></i> {{ __('buttons.edit') }}</a>
                @endif

                @if (hasPermission('employees.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('employees.destroy', encode_params($module->id)) }}', 1)"
                        class="dropdown-item"><i class="bx bx-trash"></i> {{ __('buttons.delete') }}</a>
                @endif
            @else
                @if (hasPermission('employees.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('employees.restore', encode_params($module->id)) }}', 2)"
