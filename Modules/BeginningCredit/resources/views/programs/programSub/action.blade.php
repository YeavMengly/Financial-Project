    ...
    <div class="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
            ...
            @if (is_null($module->deleted_at))
                @if (hasPermission('programSub.edit'))
                    <a href="{{ route('programSub.edit', encode_params($module->id)) }}" class="dropdown-item"><i
                            class="bx bx-edit"></i> {{ __('buttons.edit') }}</a>
                @endif
                @if (hasPermission('programSub.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('programSub.destroy', encode_params($module->id)) }}', 1)"
                        class="dropdown-item"><i class="bx bx-trash"></i> {{ __('buttons.delete') }}</a>
                @endif
            @else
                @if (hasPermission('programSub.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('programSub.restore', encode_params($module->id)) }}', 2)"
