...
@section('content')
                ...
                <div class="card-body">
                    <form class="row gx-3 gy-2 align-items-center mb-4 mb-lg-0" id="filter" method="GET">
                        ...
                        <div class="col-sm-3">
                            ...
                            <select class="form-control" name="txtSubAccount" id="txtSubAccount">
                                <option value="">{{ __('forms.search...') }}</option>
                                @foreach ($subAccount as $ts)
                                    <option value="{{ $ts->txtSubAccount }}"
                                        {{ request('txtSubAccount') == $ts->txtSubAccount ? 'selected' : '' }}>
                                        {{ $ts->txtSubAccount }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    @if (hasPermission('subAccount.create'))
                        <div class="col-sm">
                            <div class="mb-4">
                                <a class="btn btn-light waves-effect waves-light"
                                    href="{{ route('subAccount.create') }}"><i class="bx bx-plus me-1"></i>
                                    {{ __('buttons.create') }}</a>
                            </div>
                        </div>
                    @endif
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            const element = document.getElementById('subAccountNumber');
            ...
            $('#subAccountNumber').on('change', function() {
        ...
        $(document).ready(function() {
            const element = document.getElementById('txtSubAccount');
            let choicesInstance = new Choices(element, {
                searchEnabled: true,
                itemSelectText: '',
                shouldSort: false,
            ...
            $('#txtSubAccount').on('change', function() {
                const selected = $(this).val();
                let message = '';
