        ...
        <ul class="dropdown-menu dropdown-menu-end">
            @if (is_null($module->deleted_at))
                @if (hasPermission('accountSub.edit'))
                    <a href="{{ route('accountSub.edit', encode_params($module->id)) }}" class="dropdown-item"><i
                            class="bx bx-edit"></i> {{ __('buttons.edit') }}</a>
                @endif
                @if (hasPermission('accountSub.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('accountSub.destroy', encode_params($module->id)) }}', 1)"
                        class="dropdown-item"><i class="bx bx-trash"></i> {{ __('buttons.delete') }}</a>
                @endif
            @else
                @if (hasPermission('accountSub.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('accountSub.restore', encode_params($module->id)) }}', 2)"
