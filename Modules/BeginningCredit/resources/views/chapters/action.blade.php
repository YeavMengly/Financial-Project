        ...
        <ul class="dropdown-menu dropdown-menu-end">
            @if (is_null($module->deleted_at))
                @if (hasPermission('chapters.edit'))
                    <a href="{{ route('chapters.edit', encode_params($module->id)) }}" class="dropdown-item"><i
                            class="bx bx-edit"></i> {{ __('buttons.edit') }}</a>
                @endif
                @if (hasPermission('chapters.destroy'))
                    <a href="#" onclick="confirm('{{ route('chapters.destroy', encode_params($module->id)) }}', 1)"
                        class="dropdown-item"><i class="bx bx-trash"></i> {{ __('buttons.delete') }}</a>
                @endif
            @else
                @if (hasPermission('chapters.destroy'))
                    <a href="#" onclick="confirm('{{ route('chapters.restore', encode_params($module->id)) }}', 2)"
