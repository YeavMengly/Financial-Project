@if (hasPermission('depart.edit') or hasPermission('depart.destroy'))
    <div class="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
            @if (is_null($module->deleted_at))
                ...
                @if (hasPermission('subDepart.edit'))
                    <a href="{{ route('subDepart.edit', encode_params($module->id)) }}" class="dropdown-item"><i
                            class="bx bx-edit"></i> {{ __('buttons.edit') }}</a>
                @endif
                @if (hasPermission('subDepart.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('subDepart.destroy', encode_params($module->id)) }}', 1)"
                        class="dropdown-item"><i class="bx bx-trash"></i> {{ __('buttons.delete') }}</a>
                @endif
            @else
                @if (hasPermission('subDepart.destroy'))
                    <a href="#"
                        onclick="confirm('{{ route('subDepart.restore', encode_params($module->id)) }}', 2)"
