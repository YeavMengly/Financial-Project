...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-6">
            <div class="card">
                <div class="card-body">
                    <form id="pristine-valid-example" novalidate method="POST" action="{{ route('initialMandate.store') }}"
                        autocomplete="off">
                        @csrf
                        <input type="hidden" />
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group mb-3">
                                    ...
                                    <select id="cboYear" class="form-select" name="cboYear" required
                                        data-pristine-required-message="{{ __('messages.required') }}">
                                        <option value="">ជ្រើសរើស</option>
                                        @foreach ($data as $cate)
                                            <option value="{{ $cate->id }}">{{ $cate->year }}</option>
                                        @endforeach
                                    </select>
                                    @error('cboYear')
...
@section('script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const taskTypeSelect = document.getElementById('cboYear');
