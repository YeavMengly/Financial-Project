    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div>
                        <form id="pristine-valid-example" action="{{ route('budget-voucher.update', $params) }}"
                            ...
                            <div class="row">
                                        ...
                                        <select class="form-control" id="cboSubAccountNumber" name="subAccountNumber"
                                            ...
                                            @foreach ($beginCredit as $bc)
                                                <option value="{{ $bc->subAccountNumber }}"
                                                    ...
                                                    {{ old('subAccountNumber', $budgetVoucher->subAccountNumber) == $bc->subAccountNumber ? 'selected' : '' }}>
                                ...
                                <div class="col-xl-4 col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="number" min="0" name="program" id="programInput" readonly
                                            ...
                                            value="{{ old('program', $budgetVoucher->program) }}"
                                ...
                                <div class="col-xl-4 col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="number" min="0" name="budget" id="budget" required
                                            class="form-control" value="{{ old('budget', $budgetVoucher->budget) }}"
                                            data-pristine-required-message="{{ __('messages.required') }}" />
                                        @error('budget')
                                            <div class="pristine-error text-help">{{ $message }}</div>
                                        @enderror
                                        ...
                                        <select class="form-control" name="task_type" id="task_type" required
                                            ...
                                            @foreach ($taskType as $ts)
                                                <option value="{{ $ts->task }}"
                                                    {{ old('task_type', $budgetVoucher->task_type == $ts->task ? 'selected' : '') }}>
                                                    {{ $ts->task }}</option>
                                            @endforeach
                                        </select>
                                        @error('task_type')
                                ...
                                <div class="col-lg-4 col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="date" name="date" id="example-datetime-local-input" required
                                            class="form-control" value="{{ old('date', $budgetVoucher->date) }}"
                                            data-pristine-required-message="{{ __('messages.required') }}" />
                                        @error('date')
                                            <div class="pristine-error text-help">{{ $message }}</div>
                                        @enderror
                                ...
                                {{-- <div class="col-lg-4 col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        @php
                                            $existingAttachments = json_decode($budgetVoucher->attachments, true);
                                        @endphp

                                        @if (!empty($existingAttachments))
                                            <div class="mt-2">
                                                <strong>ឯកសារមុន:</strong>
                            ...
                            <div class="col-md-12">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="txtDescription" id="vDescription" rows="5" class="form-control" required
                                        data-pristine-required-message="{{ __('messages.required') }}">{{ old('txtDescription', $budgetVoucher->txtDescription) }}</textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function fetchEarlyBalance(subAccountId, programCode) {
                const baseUrl = "{{ url('budgetplan/voucher/edit') }}";
                const url = `${baseUrl}/${subAccountId}/${programCode}/early-balance`;
