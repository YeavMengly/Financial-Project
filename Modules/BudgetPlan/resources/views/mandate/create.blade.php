                ...
                <div class="card-body">
                    <div>
                        <form id="pristine-valid-example"
                            action="{{ route('budget-mandate.store', encode_params($params)) }}" method="POST"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            subAccountSelect.addEventListener('change', function() {
                ...
                const baseUrl = "{{ url('budgetplan/mandate/create') }}";
                ...
                if (subAccountId && programCode) {
                    ...
                    fetch(url)
                        ...
                        .then(data => {
                            ...
                            const finLaw = document.getElementById('fin_law');
                            const creditMovement = document.getElementById('credit_movement');
                            const newCreditStatus = document.getElementById('new_credit_status');
                            const creditDisplay = document.getElementById('credit');
                            const deadlineBalance = document.getElementById('deadline_balance');

                            if (finLaw) finLaw.textContent = formatNumber(data.fin_law);
                            if (creditMovement) creditMovement.textContent = formatNumber(data
                                .credit_movement);
                            if (newCreditStatus) newCreditStatus.textContent = formatNumber(data
                                .new_credit_status);
                            if (creditDisplay) creditDisplay.textContent = formatNumber(data.credit);
                            if (deadlineBalance) deadlineBalance.textContent = formatNumber(data
                                .deadline_balance);
