...
use App\DataTables\Budget\RoyaltyMandateDataTable;
...
class BudgetMandateController extends Controller
{
    ...
    public function getIndexRoyaltyMandate(InitialRoyaltyMandateDataTable $dataTable)
    {
        return $dataTable->render('budgetplan::royalty.initialRoyaltyMandate.index');
    }
    /**
    ...
    public function indexRoyaltyMandate(RoyaltyMandateDataTable $dataTable, $params)
    {
        $id = decode_params($params);
        $data = Ministry::where('id', $id)->first();
        ...
        return $dataTable->render('budgetplan::royalty.royaltyMandate.index', [
            'data' => $data,
            'params' => $params,
            'program' => $program,
            'accountSub' => $accountSub,
    ...
    public function createRoyaltyMandate($params)
    {
        $id = decode_params($params);
        $ministry = Ministry::where('id', $id)->first();
        $agency = Agency::where('ministry_id', $ministry->id)->get();
        $program = Program::where('ministry_id', $ministry->id)->get();
        $accountSub = AccountSub::where('ministry_id', $ministry->id)->get();
        $expenseType = ExpenseType::where('id', 7)
            ->get();
        ...
        return view('budgetplan::royalty.royaltyMandate.create')
            ->with('accountSub', $accountSub)
            ->with('agency', $agency)
            ->with('expenseType', $expenseType)
            ->with('params', $params)
    ...
    // public function storeRoyaltyMandate(Request $request, $params)
    // {
    //     $validated = $request->validate([
    //         'legalID' =>   'required',
    //         'paymentVoucher' => 'required',
    ...
    public function storeRoyaltyMandate(Request $request, $params)
    {
        ...
        try {
            ...
            return redirect()->route('royaltyMandate.index', $params);
        } catch (\Throwable $e) {
            Log::error('BudgetDirectPayment store failed: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
    ...
    public function editRoyaltyMandate($params, $id)
    {
        $id = decode_params($id);
        $ministry = Ministry::where('id', decode_params($params))->first();
        ...
        return view('budgetplan::royalty.royaltyMandate.edit')
            ->with('expenseType', $expenseType)
            ->with('accountSub', $accountSub)
            ->with('agency', $agency)
            ->with('program', $program)
    ...
    public function updateRoyaltyMandate(Request $request, $params, $id)
    {
        $validated = $request->validate([
            'legalID' =>   'required',
            'paymentVoucher' => 'required',
    ...
    public function destroyRoyaltyMandate($params, $id)
    {
        $id = decode_params($id);
        $ministry   = Ministry::where('id', decode_params($params))->first();
        $mandate = BudgetMandate::where('id', $id)
    ...
    public function restoreRoyaltyMandate($params, $id)
    {
        $pid = decode_params($id);
    ...
    public function exportRoyaltyMandate(Request $request, $params)
    {
        try {
            ...
            $query->where('budget_mandates.expense_type_id', 7);
            /**
             * Current Budget
             */
            if ($request->filled('start_date') && $request->filled('end_date')) {
