...
class EditFileMission extends Component
{
    use WithFileUploads;
    ...
    {
        $id = decode_params($id);
        $project = Mission::where("id", $id)->first();
        $this->doc_id = $project->id;

        $this->documentTitle = $project->title;
        $this->documentOldFile = $project->file;
    }

    public function render()
    {
        return view('livewire.mission.edit-file-mission');
    }
    ...
    public function save()
    {
        ...
        $path_store = "uploads/mission/" . date("Y-m-d");
        ...
        try {
            ...
            $updateDoc = Mission::findOrFail($this->doc_id);
            $updateDoc->update([
                "fileName" => $last_file
            ]);
